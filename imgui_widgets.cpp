...
#include "examples/example_win32_directx11/blur.hpp"
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, hovered ? GetDarkColor(main_color) : main_color, anim_speed);
    it_anim->second.frame_color = ImLerp(it_anim->second.frame_color, hovered ? main_color : GetColorWithAlpha(main_color, 0.3f), anim_speed * 3);

    PushStyleVar(ImGuiStyleVar_WindowRounding, style.FrameRounding);
    blur::add_blur(window->DrawList, bb.Min, bb.Max, 1.f);
    PopStyleVar();
    ...
    window->DrawList->AddText(center_text(bb.Min, bb.Max, label), GetColorU32(it_anim->second.text_color), label);
...
bool ImGui::ToggleButton(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool *toggled)
{
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, *toggled ? GetDarkColor(main_color) : main_color, anim_speed);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    if (axis == ImGuiAxis_X)
        ...
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + 10.f, bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) - 10.f + grab_h_pixels);
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, main_color, 360.f);
...
bool ImGui::Keybind(const char* label, int* key)
{
    ...
    PushStyleVar(ImGuiStyleVar_WindowRounding, style.FrameRounding);
    blur::add_blur(window->DrawList, total_bb.Min, total_bb.Max, 1.f);
    PopStyleVar();
    ...
    PushFont(small_font);
    if (label_str.find("##keybind") == std::string::npos)
        window->DrawList->AddText(ImVec2(total_bb.Min.x + 20, center_text(total_bb.Min, total_bb.Max, label).y), GetColorU32(it_anim->second.text_color), label);
    PopFont();
    ...
    if (*key > 0 && !it_anim->second.mode)
    {
        ...
        PushFont(small_font);
        window->DrawList->AddText(center_text(total_bb.Min, total_bb.Max, it_anim->second.key_name), GetColorU32(it_anim->second.key_color), it_anim->second.key_name);
        PopFont();
    }
    ...
    {
        PushFont(small_font);
        window->DrawList->AddText(center_text(total_bb.Min, total_bb.Max, "..."), GetColorU32(it_anim->second.key_color), "...");
        PopFont();
    }
    ...
    if (*key == 0 && !it_anim->second.mode)
    {
        ...
        PushFont(small_font);
        window->DrawList->AddText(center_text(total_bb.Min, total_bb.Max, "None"), GetColorU32(it_anim->second.key_color), "None");
        PopFont();
    }
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, *v ? GetColorWithAlpha(main_color, 0.3f) : second_color, anim_speed);
    it_anim->second.circle_color = ImLerp(it_anim->second.circle_color, *v ? main_color : foreground_color, anim_speed);
    it_anim->second.shadow_color = ImLerp(it_anim->second.shadow_color, *v ? main_color : GetColorWithAlpha(main_color, 0.f), anim_speed);
    it_anim->second.circle_offset = ImLerp(it_anim->second.circle_offset, *v ? 13 : 0.f, anim_speed);
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, *v ? text_color[0] : hovered ? text_color[1] : text_color[2], anim_speed);
    ...
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(it_anim->second.rect_color), 360);

    window->DrawList->AddShadowCircle(check_bb.Min + ImVec2(9 + it_anim->second.circle_offset, 8.5f), 5.5f, ImColor(0.f, 0.f, 0.f, 0.3f), 10.f, ImVec2(1, 1), 0, 360);

    window->DrawList->AddCircleFilled(check_bb.Min + ImVec2(9 + it_anim->second.circle_offset, 8.5f), 5.5f, GetColorU32(it_anim->second.circle_color), 360);

    window->DrawList->AddShadowCircle(check_bb.Min + ImVec2(9 + it_anim->second.circle_offset, 8.5f), 5.5f, GetColorU32(it_anim->second.shadow_color), 70.f, ImVec2(0,0), 0, 360);

    window->DrawList->AddText(ImVec2(total_bb.Min.x + 12.f, center_text(total_bb.Min, total_bb.Max, label).y), GetColorU32(it_anim->second.text_color), label);
...
bool ImGui::SubTab(const char* label, int* v, int number)
{
    ...
    const float square_sz = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(35 + CalcTextSize(label).x, 30));
    ...
    it_anim->second.icon_color = ImLerp(it_anim->second.icon_color, *v == number ? main_color : text_color[1], anim_speed);
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, *v == number ? text_color[0] : text_color[1], anim_speed);
    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, *v == number ? second_color : GetColorWithAlpha(second_color, 0.f), anim_speed);

    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, GetColorU32(it_anim->second.rect_color), style.FrameRounding);

    window->DrawList->AddText(center_text(total_bb.Min + ImVec2(20, 0), total_bb.Max, label), GetColorU32(it_anim->second.text_color), label);

    PushFont(small_icon_font);
    window->DrawList->AddText(ImVec2(total_bb.Min.x + 10.f, center_text(total_bb.Min, total_bb.Max, "4").y), GetColorU32(it_anim->second.icon_color), "4");
    PopFont();
...
bool ImGui::Tab(const char* label, const char* icon, int* v, int number)
{
    ...
    window->DrawList->AddText(ImVec2(total_bb.Min.x + 45, center_text(total_bb.Min, total_bb.Max, icon).y), GetColorU32(it_anim->second.text_color), label);

    PushFont(icon_big_font);
    window->DrawList->AddText(ImVec2(total_bb.Min.x + 10, center_text(total_bb.Min, total_bb.Max, icon).y), GetColorU32(it_anim->second.icon_color), icon);
    PopFont();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int value)
{
    ...
    PushFont(icon_font);
    window->DrawList->AddText(center_text(rect_bb.Max - ImVec2(25, 20), rect_bb.Max - ImVec2(4, 4), "7"), GetColorU32(it_anim->second.arrow_color), "7");
    PopFont();

    window->DrawList->AddText(total_bb.Min + ImVec2(12, 12), GetColorU32(it_anim->second.text_color), label);
    ...
    PushFont(small_font);
    window->DrawList->AddText(ImVec2(rect_bb.Min.x + 9.5f, center_text(rect_bb.Min, rect_bb.Max, preview_value).y), GetColorU32(it_anim->second.preview_text_color), preview_value);
    PopFont();
    ...
    blur::add_blur(GetWindowDrawList(), GetWindowPos(), GetWindowPos() + GetWindowSize(), 1.f);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(ImRect(slider_bb.Min - ImVec2(5, 0), slider_bb.Max + ImVec2(2, 0)), id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);

    it_anim->second.grab_bb_center_offset = ImLerp(it_anim->second.grab_bb_center_offset, grab_bb.Max - slider_bb.Min, anim_speed * 3);
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, hovered && ImGui::IsMouseDown(ImGuiMouseButton_Left) ? text_color[0] : hovered ? text_color[1] : text_color[2], anim_speed);
    ...
    if (ImGui::GetIO().MouseWheel > 0 && *(int*)p_data + 1 < *(int*)p_max && hovered)
        *(int*)p_data += ImGui::GetIO().MouseWheel;

    if (ImGui::GetIO().MouseWheel < 0 && *(int*)p_data - 1 > *(int*)p_min && hovered)
        *(int*)p_data += ImGui::GetIO().MouseWheel;

    window->DrawList->AddText(total_bb.Min + ImVec2(12, 10), GetColorU32(it_anim->second.text_color), label);

    PushFont(small_font);
    window->DrawList->AddText(center_text(input_bb.Min, input_bb.Max, value_buf), GetColorU32(it_anim->second.hint_color), value_buf);
    PopFont();
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    {
        RenderNavHighlight(frame_bb, id);
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, background_color, style.FrameRounding);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.stroke_color), style.FrameRounding);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, GetColorU32(it_anim->second.text_color), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_screen_rect.Overlaps(clip_rect))
                it_anim->second.active_input = true;
            else
                it_anim->second.active_input = false;
    ...
    it_anim->second.stroke_color = ImLerp(it_anim->second.stroke_color, it_anim->second.active_input ? main_color : stroke_color, anim_speed);
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, hovered && !it_anim->second.active_input  ? text_color[1] : it_anim->second.active_input ? main_color : text_color[2], anim_speed);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags, bool render_frame)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        ImGui::SetCursorScreenPos(total_bb.Max - ImVec2(45, 32.5f));
        ...
        it_anim->second.text_color = ImLerp(it_anim->second.text_color, it_anim->second.picker_active ? text_color[0] : ImGui::IsMouseHoveringRect(total_bb.Min, total_bb.Max, true) ? text_color[1] : text_color[2], anim_speed);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        blur::add_blur(GetWindowDrawList(), ImGui::GetWindowPos(), ImGui::GetWindowPos() + ImGui::GetWindowSize(), 1.f);
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        for (int i = 0; i < 6; ++i) {
            draw_list->AddRectFilledMultiColor(ImVec2(picker_pos.x + i * (color_frame_size.x / 6), picker_pos.y + color_frame_size.y + 10), ImVec2(picker_pos.x + (i + 1) * (color_frame_size.x / 5.95), picker_pos.y + color_frame_size.y + 20),
                col_hues[i], col_hues[i + 1], col_hues[i + 1], col_hues[i], 10.f,
                i == 0 ? ImDrawFlags_RoundCornersLeft : i == 5 ? ImDrawFlags_RoundCornersRight : ImDrawFlags_RoundCornersNone);
        }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4 & col, ImGuiColorEditFlags flags, const ImVec2 & size_arg)
{
    ...
    window->DrawList->AddRectFilled(bb.Min + ImVec2(1, 2), bb.Max - ImVec2(1, 2), GetColorU32(col_rgb), 3.f);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    PushFont(small_font);
    window->DrawList->AddText(ImVec2(bb.Min.x + 35.f, center_text(bb.Min, bb.Max, label).y), GetColorU32(it_anim->second.text_color), label);
    PopFont();
